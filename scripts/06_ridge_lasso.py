...
def run_expanding_window(df, dates, model_class, alpha, features, target):
    ...
    Returns predictions, ICs, and final model coefficients.
    """
    ic_list = []
    predictions = []
    coef_history = []
    ...
    for i in range(MIN_TRAIN_MONTHS, len(dates) - 1):
        ...
        test_copy['Date'] = test_date
        predictions.append(test_copy)
    
    return pd.DataFrame(ic_list), pd.DataFrame(coef_history), pd.concat(predictions)
...
comparison = pd.DataFrame({
    ...
    'Time_sec': [0.24, ridge_time, lasso_time]
})
