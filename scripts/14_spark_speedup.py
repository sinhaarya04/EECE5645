...
Run this AFTER running 13_spark_pipeline.py with different core counts.

Input:  outputs/tables/spark_results_*cores.csv
Output: outputs/figures/speedup_chart.png
        outputs/tables/speedup_summary.csv
"""
...
if not files:
    print("No spark results found. Run 13_spark_pipeline.py first.")
    exit()
...
# Compute speedup relative to 1 core
...
# ============================================================
# Plot 2: Absolute time chart
# ============================================================
fig, ax = plt.subplots(figsize=(12, 7))

for model in models:
    model_data = speedup_df[speedup_df['Model'] == model].sort_values('Cores')
    ax.plot(model_data['Cores'], model_data['Time_sec'], 'o-',
            label=model, linewidth=2, markersize=8,
            color=colors.get(model, 'gray'))
